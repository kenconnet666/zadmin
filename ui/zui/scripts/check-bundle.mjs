...
const components = ['provider', 'box', 'stack', 'text', 'icon', 'button', 'input', 'field'];
...
for (const component of components) {
	const componentEntry = portable(resolve(sourceRoot, `components/${component}/index.ts`));
	...
	if (incremental > 3 * 1024) {
		throw new Error(`Z${component} incremental gzip ${incremental} exceeds 3 KiB.`);
	}
	if (/node:async_hooks|compiler\/preprocess|svelte\/compiler/u.test(output.code)) {
		throw new Error(`Z${component} browser bundle contains compiler/server code.`);
	}
	report.components[component] = { gzip: output.gzip, incrementalGzip: incremental };
}
