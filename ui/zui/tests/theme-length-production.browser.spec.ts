...
describe('display component Theme lengths', () => {
	it.each([false, true])('renders numeric or CSS string lengths (strings=%s)', async (stringLengths) => {
		...
		const rectangle = document.querySelector<HTMLElement>('[data-testid="theme-length-rectangle"]')!;
		const lines = document.querySelectorAll<HTMLElement>(
			'[data-testid="theme-length-lines"] [data-slot="line"]'
		);
		const explicit = document.querySelector<HTMLElement>('[data-testid="theme-length-explicit"]')!;
