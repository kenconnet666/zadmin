<script module lang="ts">
	...
	const checkboxRecipe = defineRecipe({
		...
		variants: {
			...
			size: {
				large: (s) => {
					s.blockSize._indicatorLarge;
					s.inlineSize._indicatorLarge;
				},
				medium: () => undefined,
				small: (s) => {
					s.blockSize._indicatorSmall;
	...
	export type ZCheckboxProps = Omit<
		HTMLInputAttributes,
		'aria-checked' | 'checked' | 'defaultChecked' | 'disabled' | 'onchange' | 'type' | 'value'
	> &
		ZCheckboxVariants & {
