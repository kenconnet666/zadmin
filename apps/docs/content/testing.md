...
2026-08-29合包后的外部验收：Core、ZUI和SvelteKit三个tarball在系统临时目录安装并frozen reinstall；fixture的Svelte check为0 errors/0 warnings，adapter-node生产build与真实Node SSR通过，响应包含`data-icss` critical CSS及`style-src-elem` SHA-256，client产物不含workspace路径、compiler或server入口。真实tree-shaken browser runtime gzip为9,149 bytes，8个组件增量为677–2,655 bytes，均低于15 KiB/3 KiB预算。
