...
运行时只新增 `stylis` 和 type-only `csstype`。编译器使用 `magic-string`、`estree-walker` 和 peer `svelte/compiler`。不依赖 Emotion、dx-styles、WyW、color2k、Babel、SWC 或 ts-morph。
