...
single-pack producer 只接受没有 tracked 或 untracked 变化的干净 checkout。它生成 schema v2 `manifest.json`，把 `sourceRevision`（Git HEAD）、每个 package version、tarball bytes 与 SHA-256 固定在同一个不可变事实中。artifact reader 会先验证完整 schema，再用 `--revision=<sha>` 强制比对；CI 消费 tarball 时传入当前 `github.sha`，防止脏工作树伪装成 HEAD，或不同提交的 tarball 被交叉使用。校验失败不会触发发布，只会阻断后续 acceptance。
