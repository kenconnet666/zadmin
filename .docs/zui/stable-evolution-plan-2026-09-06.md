...
| [maturity](./component-maturity.md) / [JSON](./component-maturity.json)         | 最新已生成 141 个 authored contract、141 个视觉合同关联，DesktopVerified 0                                                            | 这些“Verified”统计来自静态合同库存；不是当前执行报告。7 个既有视觉关联缺口已准确修正并重新生成，详见第 5 节。                            |
...
| S4   | P1 / 报告可读性 | 报告明确区分 authored、executed、accepted；展示文案改成“有对应合同”，保留旧 schema 字段直到迁移所有消费者 | maturity/stability/desktop artifact/report consumers；S–M | 单靠 static stableCompliant 不可能展示“当前 CI 全绿”；无双重真相。                                |
