...
状态：无人值守实施持续中；静态审计已刷新，上一批真实Chrome定向验收已完成；本批Chrome扩展已安装启用但会话控制连接未建立，因此只记录WebStorm冷启动与浏览器合同源码，不伪称新增页面已完成真实Chrome复验；完整浏览器矩阵继续交CI且当前推送不等待（2026-09-02）
...
| 实际Demo                 |  307 | 少于2个、重复id或空源码会在`defineComponentDoc`中直接失败                                                   |
...
| 带CSS过渡的组件          |   16 | 16/16显式消费Provider motion，reduced时清除过渡                                                             |
...
最后一次按约回看的门禁：[CI run 33562104466](https://github.com/kenconnet666/zadmin/actions/runs/33562104466)，对应`c6c0953`；本批推送后不等待新CI，也不再次查询该旧运行。

- Workspace builds成功；Static contracts、Workspace tests、Docs E2E、Coverage、Packages与Windows Desktop失败，不能把可构建替代完整门禁；
- Static已从上一轮41文件103 error/13 warning收敛为7文件10 error/0 warning；本批修复Accordion联合收窄、Toaster `defaultView|null`、Form空错误映射、Mention tabindex、NumberField InputEvent、Tabs重复导出与过复杂SSR推断，并由WebStorm复核全部变更文件0 error；
- Workspace tests旧提交为736通过、159失败、2跳过。首个Drawer用例通过而后7个同文件用例失败，结合大量旧DOM命中，确认browser project缺统一cleanup；当前新增全局browser setup。共享ControllableState把不可变数组/对象深度代理后再与原值比较，触发大量`state_proxy_equality_mismatch`和选择状态漂移；当前改用`$state.raw`并固定对象身份；
- 同轮还暴露并修复冻结FormData数组的就地写入、virtual nearest错误跳边、Field control id被Popover Trigger覆盖、disabled Tooltip wrapper回调、异步Popconfirm测试焦点、Accordion Item scope隔离及多处手写事件测试未等待DOM提交；
- Docs E2E旧提交70失败中有8个已迁移Demo id仍被测试引用，FileUpload等因此直接等待不存在元素；当前静态交叉检查保证site e2e引用的Demo id全部存在。Provider density示例默认值、Command option名称、Theme token计数同步修复；ZCode切换GitHub高对比Shiki主题以消除SSR/CSP指南中3.48:1的Axe失败；
- Packages失败由重复`TabsActivationMode`声明主导，已删除旧内部路径重复导出。Coverage复用了同一ZUI测试失败，并非阈值下调问题；保留全部覆盖阈值，由下一轮远程矩阵确认收敛结果。
